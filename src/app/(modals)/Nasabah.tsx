...
import { useNasabahList } from "@/src/lib/hooks/nasabah/useNasabahList";
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.headerRow}>
        <Text style={styles.title}>Daftar Nasabah</Text>
      ...
      {items.map((row) => (
        <View key={row.id_nasabah} style={styles.card}>
          <Text style={styles.nama}>{row.nama || "-"}</Text>
          <Text style={styles.alamat}>{row.alamat || "-"}</Text>
          <Text style={styles.id}>User ID: {row.user_id || "-"}</Text>
          <Text style={styles.id}>ID Nasabah: {row.id_nasabah || "-"}</Text>
          ...
          <View style={styles.statsRow}>
            ...
            <Text style={styles.statValue}>
              {formatAngka(String(row.setor || 0))}
            </Text>
          </View>
          ...
          <View style={styles.statsRow}>
            ...
            <Text style={styles.statValue}>
              {formatAngka(String(row.angsuran || 0))}
            </Text>
          </View>
          ...
          <GradientButton
            ...
            loading={loadingSetorId === row.id_nasabah}
